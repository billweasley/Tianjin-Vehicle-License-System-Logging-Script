...
for phone_index, phone in enumerate(phone_list):
    ...
    while table is None:
        ...
        values = {'loginType': 'MOBILE',
                ...
                'grSelect': 'MOBILE',
                'mobile': phone,
                'password': password_list[phone_index],
                'validCode': out,
    ...
    final_notification_info = final_notification_info + notification_info + '\n'

requests.get("https://sc.ftqq.com/{0}.send?text={1}&desp={2}".format(ftqq_id, "小汽车摇号自动登录", final_notification_info))
